...
-- 데이터가 INSERT 되어버렸다
-- 나중에 확인을 해보니 상품이 없어서 데이토 활용 가치가 
-- 매우 떨어지는 현상이 발생했다.
-- 이런상황을 "INSERT(추가, 삽입) 이상현상"이 발생했다
--      무결성이 훼손되었다.
-- 기존의 Table을 제거하고 무결성을 유지하기 위한 "제약조건"을
--  설정을 하여 Table을 다시 만들자
...
-- 데이터를 INSERT 하는데 필수적으로 setting 되어야 할 칼럼에
-- 필수 제약조건을 설정하자
...
INSERT INTO tbl_iolist(io_date, io_buyer, io_pname, io_qty, io_price)
VALUES('2021-04-21', '이몽룡', '어묵탕' , 20 , 2000 );

INSERT INTO tbl_iolist(io_date, io_buyer, io_pname, io_qty, io_price)
VALUES('2021-04-21', '임꺽정', '바나나킥' , 50 , 4000 );
...
-- io_buyer, io_pname 칼럼만 보이고 나머지 칼럼은 숨김으로
-- 하여 보여달라
-- 보여지는 카럼에는 별명을 붙여서 보여라
SELECT io_buyer AS 고객이름, io_pname AS 구매상품 FROM tbl_iolist
WHERE io_buyer = '홍길동';
