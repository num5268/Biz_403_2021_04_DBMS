...
--      "DataBase"라고 한다.
--  생성하기 : CREATE
-- 로컬디스크의 oracle/data 폴더에 mydb.dbf라는 파일을 생성하고
-- mydb라는 이름으로 사용하겠다.
-- 데이터가 있든 없든간에 기본 크기를 10MByte로 설정해 달라
-- 만약 데이터가 추가되는 동안에 공간이 부족하면
-- 자동으로 10Kbyte씩 증가하여 준비해 달라
CREATE TABLESPACE mydb --별명
DATAFILE 'C:/oraclexe/data/mydb.dbf'-- 저장소
...
-- 일반적인 DBMS는 데이터 저장공간을 DataBase라는 것만으로
--      지정을 한다.
--      만약 데이터를 추가 등을 하고자 할대
--      어떤 DataBase에 데이터를 저장할 것인지
--      지정하지 않으면 명령 수행에 문제가 발생할수 있다
-- 오라클은 User와 TableSpace를 연결하여 사용자가 DataBase처럼
--      작동하게 할수 있다.
-- 특정 사용자로 접속만 하면 자동으로 지정된 TableSpace가 
--      사용가능한 상태가 되어 어떤 TableSpace에 데이터를
--      저장할 것인지 신경쓰지 않아도 된다.
