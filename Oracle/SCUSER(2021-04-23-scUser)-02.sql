...
-- SQL Developer에서
-- 데이터를 INSERT(추가), UPDATE(변경), DELETE(삭제) 명령을
--  수행하면
-- Oracle Server와 SQL Deverloper 사이에서 중간에
--  임시로 데이터가 보관되는 상태가된다
--  이 상태는 아직 Oracle Server의 저장소에 확정이 안된상태이다
--  확정이 안된 상태에서
--  RollBack 명령을 수행하면 
--      모든 CUD(Insert, Update, Delete)가 취소된다
-- CUD를 수행하여 데이터의 변화가 발생하면
...
-- DELETE와 UPDATE를 수행한 후 실수를 발견했을대
