...
/*
iolist 테이블과 Prod 테이블간에 상품으로 JOIN을 하여
null 값이 없는것이 확인되었다
...
-- 이 때 tbl_iolist의 상품으로 tbl_product 데이터를 조회하여
--      일치하는 데이터가 있으면
--      그 중에 상품코드 칼럼의 값을 가져와서
--      io_pcode 칼럼에 저장하라
...
SELECT io.io_pcode, io.io_pname, p.p_code,
        p.p_name, p.p_iprice, p.p_oprice
FROM tbl_iolist IO
...
-- import 된 거래처 정보와 매입매출 정보를 JOIN하여
-- null값이 있는지 확인
...
SELECT io.io_dname, io.io_dceo, d.d_ceo, d.d_name
FROM tbl_iolist IO
