...
-- 만약 author, company table에 없는 데이터가(코드)가
...
SELECT * FROM tbl_books, tbl_comp, tbl_author
...
-- tbl_books와 tbl_comp를 참조 무결성 설정
-- tbl_comp의 PK를 참조하여 tbl_books데이터에
-- 출판사 정보를 연동
-- tbl_company를 parent라고하며 REFERENES table이라고 한다
...
REFERENCES tbl_comp(cp_code); -- 누구하고,parent table(칼럼)PK
...
WHERE au_code = 'A0031';
-- "A0031" 을 삭제한 상태에서 도서정보 추가
INSERT INTO tbl_books(bk_isbn, 
                        bk_title,
                        bk_acode,
                        bk_ccode)
VALUES('970000','테스트','A0031','C0001');
...
-- 데이터를 삭제하고자 한다
-- 하지만 이미 사용된(books에 등록된) 데이터는 삭제가 불가능 하다
